...
def get_evaluation_metrics(targets, predictions, output_dict=False):
    ...
    return report, precision, f1, recall#, accuracy
...
def evaluate_intents(intent_results,
    ...
    if report_folder:
        report, precision, f1, recall = get_evaluation_metrics(
        ...
        report, precision, f1, recall = get_evaluation_metrics(targets,
                                                                 predictions)
        log_evaluation_table(report, precision, f1, recall)
...
def compute_intent_metrics(interpreter, corpus):
    ...
    targets, predictions = _targets_predictions_from(intent_results)
    _, precision, f1, recall = get_evaluation_metrics(targets, predictions)

    return {"Recall": [recall], "F1-score": [f1], "Precision": [precision]}
...
def compute_entity_metrics(interpreter, corpus):
    ...
    for extractor in extractors:
        ...
        merged_predictions = substitute_labels(merged_predictions, "O",
                                               "no_entity")
        _, precision, f1, recall = get_evaluation_metrics(merged_targets,
                                                            merged_predictions)
        ...
        entity_results[extractor]["Recall"].append(recall)
